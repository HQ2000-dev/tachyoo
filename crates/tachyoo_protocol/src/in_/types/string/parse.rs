...
use crate::in_::types::{
    string::{McStringError, str::McStr, string::McString},
    var::int::signed::parse_var_int,
};
...
pub async fn parse_mc_string<R: AsyncReadExt + Unpin, const MAX_LEN: u16>(
    ...
    let (string_len, string_len_len) = parse_var_int(reader)
        .await
...
impl<const MAX_LEN: u16> McString<MAX_LEN> {
    async fn parse_inner<R: AsyncReadExt + Unpin>(
        reader: &mut R,
        len: u16,
    ) -> Result<Self, McStringParseError> {
